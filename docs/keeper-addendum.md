...
> Job details, endpoints, upsert contract, and backoff are fully specified in
> `docs/registry-sync-spec.md`. Build to that.
...
> Refresh job and composition math are in `docs/registry-sync-spec.md` §3; the
> page layout is `design/team-admin-keepers-mockup.html`. Build to both.
