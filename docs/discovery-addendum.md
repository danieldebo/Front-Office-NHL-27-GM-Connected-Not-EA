...
- **Platform is a real filter, not a label.** Each league has a type — Xbox,
  PlayStation, or Crossplay (`league.platform`, from `db/schema-platform.sql`).
  A prospect's console determines what they can join: `console_eligible()` shows
  them their own console's leagues plus every crossplay league, and hides
  single-console leagues they can't play in. Crossplay is a first-class choice a
  commissioner makes, not an "either/both" fallback.
