...
League standings defaults and the “use league defaults” preference belong inside each version’s existing JSON settings payload; resolved season values belong in the season’s scalar snapshot columns.

**Why:** These values are part of one immutable operational-settings unit, and storing them in the existing version payload avoids making a deployment depend on a schema migration while season overrides still need queryable resolved values.

**How to apply:** Read old versions with documented defaults when the JSON keys are absent. On season creation, resolve inherited values plus explicit overrides and write the result to the season snapshot; never regenerate a schedule when applying changed defaults to a running season.
