...
import { leagueAccessFor, requireLeagueRead } from "../server/leagueAccess";
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!can(user, "rulebook:write", { kind: "league", ownerId: league.ownerId, commissionerIds: league.commissionerIds })) {
