...
let inviteId: string;
let replitIdA: string;
let replitIdB: string;
...
beforeAll(async () => {
  ...
  // 2. Two claimants
  replitIdA = `test-claimant-a-${RUN_ID}`;
  replitIdB = `test-claimant-b-${RUN_ID}`;
  await makeUser(replitIdA, `claimant-a-${RUN_ID}`);
  await makeUser(replitIdB, `claimant-b-${RUN_ID}`);
    ...
    async (ctx) => {
      ...
      const [resA, resB] = await Promise.all([
        request(app)
          ...
          .set("x-test-user-id", replitIdA)
          .send(),
        request(app)
          .post(`/api/join/${inviteToken}/claim`)
          .set("x-test-user-id", replitIdB)
          .send(),
      ]);
