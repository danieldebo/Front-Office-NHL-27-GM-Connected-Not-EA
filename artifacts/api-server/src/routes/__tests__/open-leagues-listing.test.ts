 ...
 *  3. Legacy listing platform aliases are normalized to canonical enum values
 *  4. A league with no active season cannot be listed
 *
...
afterAll(async () => {
  if (leagueId) {
    await sql(`DELETE FROM commissioner_invite WHERE league_id IN ($1, $2)`, [leagueId, noSeasonLeagueId]);
...
describe("GET /api/leagues/open — listing round-trip", () => {
  ...
  beforeAll(() => {
    mockGetCurrentUser.mockReturnValue({
      id: commAppUserId,
      appUserId: commAppUserId,
      name: `OLL Commissioner ${RUN_ID}`,
    });
  });
  ...
  it.each([
    ...
    ["xbox", "xbox"],
    ["crossplay", "crossplay"],
  ])("normalizes listing platform %s to %s", async (input, expected) => {
    if (!schemaReady) return;

    const res = await request(app)
      .put(`/api/leagues/${leagueId}/listing`)
      .send({ is_listed: true, platform: input });

    expect(res.status).toBe(200);
    expect(res.body.platform).toBe(expected);
  });

  it("validates commissioner invite rotation and preserves explicit nulls", async (ctx) => {
    if (!schemaReady) return ctx.skip();

    const invalidUses = await request(app)
      .post(`/api/leagues/${leagueId}/commissioner-invite/rotate`)
      .send({ max_uses: 0 });
    expect(invalidUses.status).toBe(400);
    ...
    const valid = await request(app)
      .post(`/api/leagues/${leagueId}/commissioner-invite/rotate`)
      .send({ max_uses: null, expires_at: null });
  ...
  it("private visibility hides a stale listing and blocks public recruitment/code lookup", async (ctx) => {
    if (!schemaReady) return ctx.skip();

    const code = `OLL${RUN_ID}`.toUpperCase();
    await sql(`UPDATE league SET visibility = 'private', public_code = $2 WHERE id = $1`, [leagueId, code]);

    expect(await getOpenLeagueIds()).not.toContain(leagueId);
    expect((await request(app).post(`/api/leagues/${leagueId}/signup`).send({})).status).toBe(404);
    expect((await request(app).post(`/api/leagues/${leagueId}/waitlist`).send({})).status).toBe(404);
    expect((await request(app).get(`/api/j/${code}`)).status).toBe(404);

    await sql(`UPDATE league SET visibility = 'public', public_code = NULL WHERE id = $1`, [leagueId]);
    expect(await getOpenLeagueIds()).toContain(leagueId);
  });
  ...
  it("rejects listing a league that has no active season", async (ctx) => {
    if (!schemaReady) return ctx.skip();
    ...
    expect(putRes.body).toMatchObject({
      status: 400,
      detail: expect.stringContaining("active season"),
    });
    expect(await getOpenLeagueIds()).not.toContain(noSeasonLeagueId);
  });
});
