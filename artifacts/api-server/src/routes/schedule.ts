...
import { requireSeasonRead } from "../server/leagueAccess";
...
async function getLeagueOwnerBySeasonId(
  seasonId: string
): Promise<{ leagueId: string; ownerId: string; commissionerIds: string[] } | null> {
  const { rows } = await pool.query<{ league_id: string; owner_user_id: string; commissioner_ids: string[] }>(
    `SELECT s.league_id, l.owner_user_id,
            COALESCE(array_agg(m.user_id) FILTER (WHERE m.left_at IS NULL
              AND m.role IN ('commissioner', 'assistant_commissioner')), '{}') AS commissioner_ids
       FROM season s
       JOIN league l ON l.id = s.league_id
        LEFT JOIN league_membership m ON m.league_id = l.id
       WHERE s.id = $1 GROUP BY s.league_id, l.id`,
    [seasonId]
  );
  const row = rows[0];
  if (!row) return null;
  return { leagueId: row.league_id, ownerId: row.owner_user_id, commissionerIds: row.commissioner_ids };
}

async function getLeagueOwnerByGameId(
  gameId: string
): Promise<{ leagueId: string; ownerId: string; seasonId: string; commissionerIds: string[] } | null> {
  const { rows } = await pool.query<{
    league_id: string; owner_user_id: string; season_id: string; commissioner_ids: string[];
  }>(
    `SELECT s.league_id, l.owner_user_id, g.season_id,
       ...
       JOIN season s ON s.id = g.season_id
        JOIN league l ON l.id = s.league_id
        LEFT JOIN league_membership m ON m.league_id = l.id
       WHERE g.id = $1 GROUP BY s.league_id, l.id, g.season_id`,
    [gameId]
  );
  const row = rows[0];
  if (!row) return null;
  return { leagueId: row.league_id, ownerId: row.owner_user_id, seasonId: row.season_id, commissionerIds: row.commissioner_ids };
}
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!can(user, "schedule:generate", { kind: "league", ownerId: league.ownerId, commissionerIds: league.commissionerIds })) {
    ...
    const configuredGamesPerMatchup = Number(season.games_per_matchup);
    ...
    try {
      ...
      await publishDomainEvent(client, {
        eventType: "schedule.generated", leagueId: league.leagueId,
        actorUserId: actorAppUserId, title: "Schedule published",
        body: `${result.games.length} games were scheduled.`,
        data: { season_id: seasonId, games_created: result.games.length },
      });
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!can(user, "game:manage", { kind: "league", ownerId: league.ownerId, commissionerIds: league.commissionerIds })) {
    ...
    try {
      ...
      await publishDomainEvent(client, {
        eventType: "schedule.window_shifted", leagueId: league.leagueId,
        actorUserId: user.appUserId, title: "Game window changed",
        body: reason, data: { game_id: gameId },
      });
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      await publishDomainEvent(client, {
        eventType: "schedule.game_postponed", leagueId: league.leagueId,
        actorUserId: user.appUserId, title: "Game postponed",
        body: reason, data: { game_id: gameId },
      });
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      await publishDomainEvent(client, {
        eventType: "schedule.game_resolved", leagueId: league.leagueId,
        actorUserId: user.appUserId, title: "Game force-resolved",
        body: reason, data: { game_id: gameId, resolution },
      });
