...
import { AssignGmBody, CreateInviteBody, ListJoinRequestsParams, ListJoinRequestsQueryParams } from "@workspace/api-zod";
...
async function getLeagueOwner(leagueId: string) {
  const row = await pool.query<{ id: string; owner_user_id: string; commissioner_ids: string[] }>(
    `SELECT l.id, l.owner_user_id,
       ...
       FROM league l LEFT JOIN league_membership m ON m.league_id = l.id
  ...
  const { rows } = await pool.query(
    `SELECT
       ...
       to_jsonb(au)->>'psn_online_id' AS gm_psn_online_id,
        to_jsonb(au)->>'xbox_identity_verified_at' AS gm_xbox_identity_verified_at,
        to_jsonb(au)->>'playstation_identity_verified_at' AS gm_playstation_identity_verified_at,
          ...
          user_id: r.gm_user_id,
           ...canonicalGmIdentity({
             ...
             gm_psn_online_id: r.gm_psn_online_id as string | null,
              gm_xbox_identity_verified_at: r.gm_xbox_identity_verified_at as string | null,
              gm_playstation_identity_verified_at: r.gm_playstation_identity_verified_at as string | null,
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!can(user, "invite:manage", { kind: "league", ownerId: league.owner_user_id, commissionerIds: league.commissioner_ids })) {
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const paramsInput = ListJoinRequestsParams.safeParse(req.params);
    const query = ListJoinRequestsQueryParams.safeParse(req.query);
    if (!paramsInput.success || !query.success) {
      badRequest(res, "Invalid path or query parameters");
      return;
    }
    const leagueId = paramsInput.data.leagueId;
    const league = await getLeagueOwner(leagueId);
    if (!league) { notFound(res, "League not found"); return; }

    if (!can(user, "seat:manage", { kind: "league", ownerId: league.owner_user_id, commissionerIds: league.commissioner_ids })) {
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      await client.query("BEGIN");
      await client.query(
        `SELECT id FROM team_season WHERE id = $1 FOR UPDATE`,
        [teamSeasonId],
      );
