...
const identityVerificationBucket = new TokenBucket(6, 1 / 60);

// Prune stale entries every 10 minutes.
setInterval(() => {
  userBucket.prune();
  leagueBucket.prune();
  identityVerificationBucket.prune();
}, 10 * 60 * 1000).unref();

export function identityVerificationRateLimiter(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  ...
  const { allowed, retryAfterSecs } = identityVerificationBucket.consume(userId);
  if (!allowed) {
    ...
    res.status(429).json({
      ...
      status: 429,
      detail: `Identity verification limit exceeded. Try again in ${retryAfterSecs}s.`,
