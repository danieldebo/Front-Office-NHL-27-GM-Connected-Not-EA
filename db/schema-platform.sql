...
-- carried a loose TEXT `platform` field; this normalizes it to one vocabulary so
...
VALUES ('1.5.0', 'League platform vocabulary: xbox / playstation / crossplay')
...
ALTER TABLE league
    ADD COLUMN IF NOT EXISTS platform TEXT NOT NULL DEFAULT 'crossplay';
...
-- columns with comments like 'psn' | 'xbox' | 'both'. Normalize those values
-- in place while retaining TEXT columns so Publish never needs to alter a
-- view-backed column's PostgreSQL type.
